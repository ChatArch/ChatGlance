...
登录后的账号控制小窗另有“检查并用卡”：按钮初始禁用，只有确认处理器安装后才启用；点击后须在浏览器明确确认，处理器才向同源 `POST /use-credit` 提交账号、会话绑定的一次性 CSRF 和确认值。原生表单没有预填确认值，无 JavaScript 的提交也被服务端拒绝。取消、打开小窗、普通刷新、GET 不用卡。服务端重新读取当前 ChatEnv 策略和 CRS 账号映射，查询新的额度、卡片详情及配置要求的预测；达到当前配置的阈值、剩余时间等全部保护条件且有确切可用卡时，才复用定时执行的账号幂等账本尝试最多一次兑换。页面显示检查时间、成功/未满足/结果待确认；超时或结果不明不会自动重试，须先核实账本与额度。客户端不保存 CRS 密钥，也不读取本机 OAuth。服务端固定原因码不会包含上游响应或凭据。
