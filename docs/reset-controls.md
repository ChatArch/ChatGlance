...
- **已暂停**：该账号开关关闭，不会自动用卡。
- **等待条件**：账号已开启，但条件未满足、数据不足或存在执行保护。
- **已就绪**：账号开关与快照条件通过；下一次正常定时检查会重新核验，仍满足才自动用卡，不是在页面里立即兑换。

账号开关与业务条件列在同一执行清单中。关闭账号改变总体状态，不伪造原业务条件；修改一个账号不改变其他账号，也不使其他账号的表单版本失效。
...
普通`account-limits collect`按各账号的`enabled`判断许可；缺省策略关闭。单次只读检查使用`--no-execute-resets`；`chatglance refresh`始终不消费。单次检查模式不是账号设置，不应展示为当前账号的“仅预演”。
