...
4. **Generated Glance YAML** is rendered from the JSON and inserted into a candidate `glance.yml`.
5. The candidate config is validated before replacing the live config; service-manager actions stay in the outer scheduler/wrapper.
...
A server appears on the Infra/`服务器` page only when it is selected by the server inventory config or by the safe default candidate filter.
...
- `server-status.json`: read-only SSH probe output for the current snapshot.
- `server-page.yml`: rendered Glance page object for the Infra/`服务器` page.
...
If validation passes and the candidate differs from the live config, back up `glance.yml`, move the candidate into place, and let the outer scheduler or operator perform the service-manager action when needed:
